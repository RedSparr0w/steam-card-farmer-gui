...
SteamUser.prototype.gamesPlayed = function(apps) {
	var toplay = [];
	if (typeof apps == "object" || typeof apps == "array"){
		apps.forEach(function(i){
			if (typeof i == "string"){
				var app = {"game_id": "15190414816125648896","game_extra_info": i}
			}else{
				var app = {"game_id": i,"game_extra_info": "Some Game"}
			}
			toplay.push(app);
		});
	}else if (typeof apps == "string"){
		var toplay = [{"game_id": "15190414816125648896","game_extra_info": apps}];
	}else{
		var toplay = [{"game_id": apps,"game_extra_info": "Some Game"}];
	}
	this._send(SteamUser.EMsg.ClientGamesPlayed,toplay);
};
